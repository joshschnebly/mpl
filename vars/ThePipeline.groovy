    ...
    stages {
      ...
      stage('Version Server') {
        when { 
          ...
          branch pattern: (MPL.moduleConfig('VersionServer').'when_branch' ? "${MPL.moduleConfig('VersionServer').'when_branch'}": "*."), comparator: "REGEXP"
        }
        steps { MPLModule() }
      }
      stage('Build') {
        when { 
          expression { MPLModuleEnabled() } 
          branch pattern: (MPL.moduleConfig('Build').'when_branch' ? "${MPL.moduleConfig('Build').'when_branch'}": "*."), comparator: "REGEXP"
        }
        parallel {    
          stage('Build Server') {
            when { expression { MPLModuleEnabled() } }
